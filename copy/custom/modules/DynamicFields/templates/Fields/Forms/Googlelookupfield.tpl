...
<tr>
    <td class='mbLBL'>{sugar_translate module="DynamicFields" label="LBL_GOOGLELOOKUPFIELD_TYPE"}:</td>
    <td>
        {if $hideLevel < 5}
            {html_options name="ext1" id="ext1" selected=$googleLookupType options=$googleLookupTypes}
        {else}
            <input type='hidden' id='ext1' name='ext1' value='{$googleLookupType}'>
            {$googleLookupTypeName}
        {/if}
    </td>
</tr>
<tr>
    <td class='mbLBL'>{sugar_translate module="DynamicFields" label="LBL_GOOGLELOOKUPFIELD_COUNTRY"}:</td>
    <td>
        {if $hideLevel < 5}
            {html_options name="ext2" id="ext2" selected=$googleFilterCountry options=$googleFilterCountries}
        {else}
            <input type='hidden' id='ext2' name='ext2' value='{$googleFilterCountry}'>
            {$googleFilterCountryName}
        {/if}
    </td>
</tr>
<tr>
<td class='mbLBL'>{sugar_translate module="DynamicFields" label="LBL_GOOGLELOOKUPFIELD_POPULATE"}:</td>
<td>
        {if $hideLevel < 5}
            {html_options name="ext3" id="ext3" selected=$googleFilterPopulate options=$googleLookupPopulateOptions}
        {else}
            <input type='hidden' id='ext3' name='ext3' value='{$googleFilterPopulate}'>
            {$googleFilterPopulateName}
        {/if}
</td>
</tr>
